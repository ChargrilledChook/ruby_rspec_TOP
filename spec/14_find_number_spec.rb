...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:number_double) { double('number_double', value: 6) }
    subject(:game_solution) { described_class.new(0, 9, number_double, 6) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        expect(game_solution).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_solution) { described_class.new(0, 9, number_double, 4) }
      it 'is not game over' do
        expect(game_solution).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect(game_edge.min).to eq(game_edge.max)
      end

      it 'does not update max' do
        expect(game_edge.max).to eq(8)
      end
    end
  end
end
