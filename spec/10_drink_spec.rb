...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:me_beer) { described_class.new('beer', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(me_beer.type).to eq('beer')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(me_beer.ounces).to eq 16
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:me_beer) { described_class.new('beer', 10) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(me_beer).to_not be_full
      end
    end
  end
end
