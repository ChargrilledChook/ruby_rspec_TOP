...
describe Dog do
  ...
  subject(:tali) { described_class.new('Tali', 'Mutt', 'Tan') }
  ...
  it 'is not bwoofing' do
    #expect(tali.barking?).to false
    expect(tali).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(tali).to be_sleeping
  end
end
